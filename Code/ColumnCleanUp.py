# -*- coding: utf-8 -*-
"""
Created on Thu Dec  2 09:34:54 2021

@author: jtschulberg / rkelley05

This script is meant to clean up the columns in the Adoptions dataset.
...
#    cleaned_df = cleaned_df[['DOG NAME', 'BREED MIXES', 'COLOR']]
    ...
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.lower().str.contains('screen').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.lower().str.contains('walks alone').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.lower().str.contains('month old puppy').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.lower().str.contains('new this week').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.lower().str.contains('kids').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.lower().str.contains('energy').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.contains('HW').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.lower().str.contains('nervous').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    cleaned_df.loc[cleaned_df['COLOR'].str.lower().str.lower().str.contains('retriever').replace(np.nan, False), 'COLOR_FIXED'] = cleaned_df['BREED MIXES']
    ...
    cleaned_df.loc[(cleaned_df['COLOR_FIXED'].str.contains('black')) & ~(cleaned_df['COLOR_FIXED'].isna()), 'contains_black'] = 1
    cleaned_df.loc[~(cleaned_df['COLOR_FIXED'].str.contains('black').replace(np.nan, False)) & ~(cleaned_df['COLOR_FIXED'].isna()), 'contains_black'] = 0
    cleaned_df.loc[(cleaned_df['COLOR_FIXED'].str.contains('white')) & ~(cleaned_df['COLOR_FIXED'].isna()), 'contains_white'] = 1
    cleaned_df.loc[~(cleaned_df['COLOR_FIXED'].str.contains('white').replace(np.nan, False)) & ~(cleaned_df['COLOR_FIXED'].isna()), 'contains_white'] = 0
    # Create a boolean for tan/yellow/golden
    yellow_bool = (cleaned_df['COLOR_FIXED'].str.contains('tan')) | (cleaned_df['COLOR_FIXED'].str.contains('yellow')) | (cleaned_df['COLOR_FIXED'].str.contains('golden'))
